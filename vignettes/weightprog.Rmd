...
In addition, in the future, we might have interested in further evidence for classifying our observations. Since we have numerous classes for our objective variable, we can observe how new observations are classified by the characteristics we are focusing on. This type of classiciation, and group selection can provide further evidence on whether there are distinct characteristics that are in one group as to another. This is important and different from the k-means clustering performed above, because unlike the cluster analysis which is an exploratory method to determine a pattern for grouping certain variables, the discriminant analysis allows us to allocate each of them to a group in which groups are known in advance. This will allow us to observe how the group separation takes place. 
